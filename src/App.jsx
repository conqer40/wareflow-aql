...
export function App() {
  ...
  const content=useMemo(()=>{ if(page==="dashboard") return <Dashboard onNavigate={setPage}/>; if(page==="qc") return <QCPage onToast={showToast} onDocumentsChanged={refreshDocuments}/>; if(page==="transfers") return <TransferPage onToast={showToast} onDocumentsChanged={refreshDocuments}/>; if(["receipts","issues","counts","waste"].includes(page)) return <DocumentsPage kind={page} documents={documents} onToast={showToast} onCreate={()=>setModal(true)}/>; if(page==="balances"||page==="ledger") return <InventoryPage ledger={page==="ledger"} onToast={showToast}/>; if(page==="users") return <UsersPage onToast={showToast} currentUser={user}/>; if(["items","warehouses","suppliers"].includes(page)) return <MasterPage type={page} onToast={showToast}/>; if(page==="approvals") return <Approvals documents={documents} onDecision={decide}/>; if(page==="reports") return <Reports onToast={showToast}/>; return <Dashboard onNavigate={setPage}/>},[page,documents,user]);
