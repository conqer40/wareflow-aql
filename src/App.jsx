...
const navGroups = [
  ...
  { title: "الرقابة", items: [
    ...
    { id: "approvals", label: "الاعتمادات", icon: ShieldCheck, badge: 12 }, { id: "settings", label: "الإعدادات العامة", icon: Gear }, { id: "treasury", label: "يوميات الخزنة", icon: FileText },
    { id: "parties", label: "أرصدة العملاء والموردين", icon: Users }, { id: "reports", label: "التقارير", icon: ChartBar },
  ]},
];
...
export function App() {
  ...
  const content=useMemo(()=>{ if(page==="dashboard") return <Dashboard onNavigate={setPage}/>; if(page==="qc") return <QCPage onToast={showToast} onDocumentsChanged={refreshDocuments}/>; if(page==="transfers") return <TransferPage onToast={showToast} onDocumentsChanged={refreshDocuments}/>; if(page==="issues") return <AdvancedSalesPage onToast={showToast}/>; if(page==="treasury") return <TreasuryPage onToast={showToast}/>; if(page==="parties") return <PartyBalancesPage onToast={showToast}/>; if(page==="settings") return <SettingsPage onToast={showToast}/>; if(["receipts","counts","waste"].includes(page)) return <DocumentsPage kind={page} documents={documents} onToast={showToast} onCreate={()=>setModal(true)}/>; if(page==="balances"||page==="ledger") return <InventoryPage ledger={page==="ledger"} onToast={showToast}/>; if(page==="users") return <UsersPage onToast={showToast} currentUser={user}/>; if(["items","warehouses","suppliers"].includes(page)) return <EnhancedMasterPage type={page} onToast={showToast}/>; if(page==="approvals") return <Approvals documents={documents} onDecision={decide}/>; if(page==="reports") return <Reports onToast={showToast}/>; return <Dashboard onNavigate={setPage}/>},[page,documents,user]);
...
const TREASURY_KEY = "wareflow_treasury_v1";
const SETTINGS_KEY = "wareflow_settings_v1";
...
function getTreasury(){ const saved=localStorage.getItem(TREASURY_KEY); if(saved)return JSON.parse(saved); const rows=[]; localStorage.setItem(TREASURY_KEY,JSON.stringify(rows)); return rows; }
function setTreasury(rows){ localStorage.setItem(TREASURY_KEY,JSON.stringify(rows)); }
function getSettings(){ const saved=localStorage.getItem(SETTINGS_KEY); if(saved)return JSON.parse(saved); const s={companyName:"عقل للأدوات الصحية",taxNo:"",address:"",phone:"01023299755",defaultWarehouse:"المخزن الرئيسي",defaultCashbox:"الخزنة الرئيسية",invoicePrefix:"INV",currency:"ج.م",requireQc:true}; localStorage.setItem(SETTINGS_KEY,JSON.stringify(s)); return s; }
...
function AdvancedSalesPage({onToast}) {
  const [entities,setEntitiesState]=useState(()=>getEntities());
  const [rows,setRows]=useState(()=>getSales());
  const [query,setQuery]=useState("");
  const [open,setOpen]=useState(false);
  const firstItem=entities.items[0]||{};
  const [form,setForm]=useState({customer:"عميل نقدي",paymentType:"نقدي",warehouse:"المخزن الرئيسي",itemCode:firstItem.code||"",qty:1,price:firstItem.price||0,notes:""});
  const filtered=rows.filter(r=>Object.values(r).join(" ").toLowerCase().includes(query.toLowerCase()));
  const selectedItem=entities.items.find(x=>x.code===form.itemCode)||firstItem;
  const total=Number(form.qty||0)*Number(form.price||0);
  const create=e=>{e.preventDefault(); const selected=entities.items.find(x=>x.code===form.itemCode); if(!selected)return onToast("اختار صنف صحيح"); if(Number(form.qty)>Number(selected.onHand||0))return onToast("الرصيد غير كافي"); const sale={id:Date.now(),no:`INV-2026-${String(rows.length+1).padStart(6,"0")}`,date:new Date().toLocaleDateString("ar-EG"),type:"فاتورة بيع",customer:form.customer,paymentType:form.paymentType,warehouse:form.warehouse,itemCode:selected.code,itemName:selected.name,qty:Number(form.qty),price:Number(form.price||0),total,status:form.paymentType==="نقدي"?"مدفوعة":"آجل",notes:form.notes}; const nextRows=[sale,...rows]; const nextEntities={...entities,items:entities.items.map(x=>x.code===selected.code?{...x,onHand:Number(x.onHand||0)-Number(form.qty)}:x)}; if(form.paymentType==="نقدي"){const treasury=[{id:Date.now()+1,date:sale.date,ref:sale.no,direction:"وارد",partyType:"عميل",partyName:sale.customer,category:"تحصيل فاتورة بيع نقدي",amount:sale.total,notes:sale.itemName},...getTreasury()]; setTreasury(treasury)} setSales(nextRows); setEntities(nextEntities); setRows(nextRows); setEntitiesState(nextEntities); setOpen(false); setForm({customer:"عميل نقدي",paymentType:"نقدي",warehouse:form.warehouse,itemCode:selected.code,qty:1,price:selected.price||0,notes:""}); onToast(form.paymentType==="نقدي"?"تم تسجيل الفاتورة وتوريد المبلغ للخزنة":"تم تسجيل فاتورة آجلة على حساب العميل");};
  return <><PageTitle title="فواتير البيع وأذون الصرف" subtitle="كل فاتورة باسم العميل، نقدي أو آجل، مع خصم المخزون وتحديث الخزنة/حساب العميل" action={open?"إغلاق النموذج":"فاتورة بيع جديدة"} onAction={()=>setOpen(!open)} icon={ClipboardText}/>
    {open&&<section className="panel inline-form"><form className="form-grid" onSubmit={create}><label>اسم العميل<input value={form.customer} onChange={e=>setForm({...form,customer:e.target.value})} required/></label><label>نوع الفاتورة<select value={form.paymentType} onChange={e=>setForm({...form,paymentType:e.target.value})}><option>نقدي</option><option>آجل</option></select></label><label>المخزن<select value={form.warehouse} onChange={e=>setForm({...form,warehouse:e.target.value})}>{entities.warehouses.map(w=><option key={w.id}>{w.name}</option>)}</select></label><label>الصنف<select value={form.itemCode} onChange={e=>{const it=entities.items.find(x=>x.code===e.target.value);setForm({...form,itemCode:e.target.value,price:it?.price||0})}}>{entities.items.map(x=><option key={x.code} value={x.code}>{x.code} - {x.name} / رصيد {x.onHand}</option>)}</select></label><label>الكمية<input type="number" min="1" value={form.qty} onChange={e=>setForm({...form,qty:e.target.value})} required/></label><label>سعر البيع<input type="number" min="0" value={form.price} onChange={e=>setForm({...form,price:e.target.value})}/></label><label>إجمالي الفاتورة<input readOnly value={money.format(total)}/></label><label>الرصيد الحالي للصنف<input readOnly value={selectedItem.onHand??0}/></label><label className="full">ملاحظات<textarea value={form.notes} onChange={e=>setForm({...form,notes:e.target.value})}/></label><div className="form-actions full"><button className="primary"><Plus/> حفظ الفاتورة</button></div></form></section>}
    <Toolbar query={query} setQuery={setQuery} onExport={()=>downloadCsv("sales-invoices",[["رقم الفاتورة","التاريخ","العميل","نقدي/آجل","الصنف","الكمية","الإجمالي","الحالة"],...filtered.map(r=>[r.no,r.date,r.customer,r.paymentType,r.itemName,r.qty,r.total,r.status])])} onFilter={()=>setQuery("")}/>
    <section className="panel"><div className="summary-strip"><div><span>إجمالي الفواتير</span><b>{money.format(rows.reduce((s,r)=>s+Number(r.total||0),0))} ج.م</b></div><div><span>نقدي</span><b>{money.format(rows.filter(r=>r.paymentType==="نقدي").reduce((s,r)=>s+Number(r.total||0),0))}</b></div><div><span>آجل</span><b>{money.format(rows.filter(r=>r.paymentType==="آجل").reduce((s,r)=>s+Number(r.total||0),0))}</b></div></div><div className="table-wrap"><table><thead><tr><th>رقم الفاتورة</th><th>العميل</th><th>نوعها</th><th>الصنف</th><th>الكمية</th><th>الإجمالي</th><th>الحالة</th></tr></thead><tbody>{filtered.length?filtered.map(r=><tr key={r.id}><td><b className="link">{r.no}</b><small>{r.date}</small></td><td>{r.customer}</td><td>{r.paymentType}</td><td><b>{r.itemName}</b><small>{r.itemCode}</small></td><td>{r.qty}</td><td>{money.format(r.total)} ج.م</td><td><Badge>{r.status}</Badge></td></tr>):<tr><td colSpan="7"><div className="empty-state">لا توجد فواتير بيع حتى الآن</div></td></tr>}</tbody></table></div></section>
  </>;
}

function TreasuryPage({onToast}) {
  const [rows,setRows]=useState(()=>getTreasury());
  const [query,setQuery]=useState("");
  const [open,setOpen]=useState(false);
  const [form,setForm]=useState({direction:"وارد",partyType:"عميل",partyName:"",category:"تحصيل",amount:"",ref:"",notes:""});
  const filtered=rows.filter(r=>Object.values(r).join(" ").toLowerCase().includes(query.toLowerCase()));
  const income=rows.filter(r=>r.direction==="وارد").reduce((s,r)=>s+Number(r.amount||0),0);
  const outcome=rows.filter(r=>r.direction==="منصرف").reduce((s,r)=>s+Number(r.amount||0),0);
  const save=e=>{e.preventDefault(); const row={id:Date.now(),date:new Date().toLocaleDateString("ar-EG"),...form,amount:Number(form.amount||0)}; const next=[row,...rows]; setTreasury(next); setRows(next); setOpen(false); setForm({...form,partyName:"",amount:"",ref:"",notes:""}); onToast("تم تسجيل حركة الخزنة");};
  return <><PageTitle title="يوميات الخزنة" subtitle="تسجيل الوارد والمنصرف ومعرفة صافي رصيد الخزنة لحظة بلحظة" action={open?"إغلاق النموذج":"حركة خزنة جديدة"} onAction={()=>setOpen(!open)} icon={FileText}/>
    <section className="summary-strip"><div><span>الوارد</span><b>{money.format(income)} ج.م</b></div><div><span>المنصرف</span><b>{money.format(outcome)} ج.م</b></div><div><span>صافي الخزنة</span><b>{money.format(income-outcome)} ج.م</b></div></section>
    {open&&<section className="panel inline-form"><form className="form-grid" onSubmit={save}><label>نوع الحركة<select value={form.direction} onChange={e=>setForm({...form,direction:e.target.value})}><option>وارد</option><option>منصرف</option></select></label><label>الطرف<select value={form.partyType} onChange={e=>setForm({...form,partyType:e.target.value})}><option>عميل</option><option>مورد</option><option>عام</option></select></label><label>اسم الطرف<input value={form.partyName} onChange={e=>setForm({...form,partyName:e.target.value})} required/></label><label>البند<input value={form.category} onChange={e=>setForm({...form,category:e.target.value})} required/></label><label>المبلغ<input type="number" min="0" value={form.amount} onChange={e=>setForm({...form,amount:e.target.value})} required/></label><label>مرجع / رقم فاتورة<input value={form.ref} onChange={e=>setForm({...form,ref:e.target.value})}/></label><label className="full">ملاحظات<textarea value={form.notes} onChange={e=>setForm({...form,notes:e.target.value})}/></label><div className="form-actions full"><button className="primary"><Plus/> حفظ الحركة</button></div></form></section>}
    <Toolbar query={query} setQuery={setQuery} onExport={()=>downloadCsv("treasury",[["التاريخ","النوع","الطرف","الاسم","البند","المبلغ","المرجع"],...filtered.map(r=>[r.date,r.direction,r.partyType,r.partyName,r.category,r.amount,r.ref])])} onFilter={()=>setQuery("")}/>
    <section className="panel"><div className="table-wrap"><table><thead><tr><th>التاريخ</th><th>وارد/منصرف</th><th>الطرف</th><th>البند</th><th>المبلغ</th><th>المرجع</th></tr></thead><tbody>{filtered.length?filtered.map(r=><tr key={r.id}><td>{r.date}</td><td><Badge>{r.direction}</Badge></td><td><b>{r.partyName}</b><small>{r.partyType}</small></td><td>{r.category}</td><td>{money.format(r.amount)} ج.م</td><td>{r.ref||"—"}</td></tr>):<tr><td colSpan="6"><div className="empty-state">لا توجد حركات خزنة بعد</div></td></tr>}</tbody></table></div></section>
  </>;
}

function PartyBalancesPage({onToast}) {
  const sales=getSales(), treasury=getTreasury();
  const names=[...new Set([...sales.map(x=>x.customer),...treasury.map(x=>x.partyName)].filter(Boolean))];
  const rows=names.map(name=>{const creditSales=sales.filter(s=>s.customer===name&&s.paymentType==="آجل").reduce((a,b)=>a+Number(b.total||0),0); const customerReceipts=treasury.filter(t=>t.partyName===name&&t.partyType==="عميل"&&t.direction==="وارد").reduce((a,b)=>a+Number(b.amount||0),0); const supplierPayments=treasury.filter(t=>t.partyName===name&&t.partyType==="مورد"&&t.direction==="منصرف").reduce((a,b)=>a+Number(b.amount||0),0); const supplierDues=treasury.filter(t=>t.partyName===name&&t.partyType==="مورد"&&t.direction==="وارد").reduce((a,b)=>a+Number(b.amount||0),0); return {name,customerBalance:creditSales-customerReceipts,supplierBalance:supplierDues-supplierPayments,creditSales,customerReceipts,supplierPayments,supplierDues};});
  const totalCustomers=rows.reduce((s,r)=>s+r.customerBalance,0), totalSuppliers=rows.reduce((s,r)=>s+r.supplierBalance,0);
  return <><PageTitle title="أرصدة العملاء والموردين" subtitle="رصيد كل عميل وكل مورد بشكل منفصل مع الإجماليات المجمعة" action="تصدير" onAction={()=>downloadCsv("party-balances",[["الطرف","رصيد عميل","رصيد مورد","فواتير آجلة","تحصيلات","مدفوعات مورد"],...rows.map(r=>[r.name,r.customerBalance,r.supplierBalance,r.creditSales,r.customerReceipts,r.supplierPayments])])} icon={Users}/>
    <section className="summary-strip"><div><span>إجمالي مديونية العملاء</span><b>{money.format(totalCustomers)} ج.م</b></div><div><span>إجمالي أرصدة الموردين</span><b>{money.format(totalSuppliers)} ج.م</b></div><div><span>عدد الأطراف</span><b>{rows.length}</b></div></section>
    <section className="panel"><div className="table-wrap"><table><thead><tr><th>الطرف</th><th>رصيد العميل</th><th>رصيد المورد</th><th>فواتير آجلة</th><th>تحصيلات عميل</th><th>مدفوعات مورد</th></tr></thead><tbody>{rows.length?rows.map(r=><tr key={r.name}><td><b>{r.name}</b></td><td>{money.format(r.customerBalance)} ج.م</td><td>{money.format(r.supplierBalance)} ج.م</td><td>{money.format(r.creditSales)}</td><td>{money.format(r.customerReceipts)}</td><td>{money.format(r.supplierPayments)}</td></tr>):<tr><td colSpan="6"><div className="empty-state">لا توجد أرصدة حتى الآن</div></td></tr>}</tbody></table></div></section>
  </>;
}

function SettingsPage({onToast}) {
  const [settings,setLocalSettings]=useState(()=>getSettings());
  const save=e=>{e.preventDefault(); setSettings(settings); onToast("تم حفظ الإعدادات العامة");};
  const exportData=()=>{const payload={settings:getSettings(),entities:getEntities(),sales:getSales(),treasury:getTreasury(),store:loadStore(),exportedAt:new Date().toISOString()}; const url=URL.createObjectURL(new Blob([JSON.stringify(payload,null,2)],{type:"application/json"})); const a=document.createElement("a"); a.href=url; a.download="wareflow-backup.json"; a.click(); URL.revokeObjectURL(url); onToast("تم تجهيز نسخة احتياطية من البيانات");};
  const resetData=()=>{ if(!confirm("هل تريد مسح بيانات التشغيل من هذا المتصفح؟"))return; [ENTITY_KEY,SALES_KEY,TREASURY_KEY,STORE_KEY].forEach(k=>localStorage.removeItem(k)); onToast("تم مسح بيانات التشغيل، أعد تحميل الصفحة"); };
  return <><PageTitle title="الإعدادات العامة" subtitle="بيانات الشركة، إعدادات الفواتير، الخزنة الافتراضية، والنسخ الاحتياطي" action="حفظ الإعدادات" onAction={()=>setSettings(settings)} icon={Gear}/>
    <section className="panel inline-form"><form className="form-grid" onSubmit={save}><label>اسم الشركة<input value={settings.companyName} onChange={e=>setLocalSettings({...settings,companyName:e.target.value})}/></label><label>رقم ضريبي / سجل<input value={settings.taxNo} onChange={e=>setLocalSettings({...settings,taxNo:e.target.value})}/></label><label>الهاتف<input value={settings.phone} onChange={e=>setLocalSettings({...settings,phone:e.target.value})}/></label><label>العملة<input value={settings.currency} onChange={e=>setLocalSettings({...settings,currency:e.target.value})}/></label><label>المخزن الافتراضي<input value={settings.defaultWarehouse} onChange={e=>setLocalSettings({...settings,defaultWarehouse:e.target.value})}/></label><label>الخزنة الافتراضية<input value={settings.defaultCashbox} onChange={e=>setLocalSettings({...settings,defaultCashbox:e.target.value})}/></label><label>بادئة الفواتير<input value={settings.invoicePrefix} onChange={e=>setLocalSettings({...settings,invoicePrefix:e.target.value})}/></label><label>فحص الجودة قبل الإضافة<select value={settings.requireQc?"yes":"no"} onChange={e=>setLocalSettings({...settings,requireQc:e.target.value==="yes"})}><option value="yes">مفعل</option><option value="no">غير مفعل</option></select></label><label className="full">العنوان<textarea value={settings.address} onChange={e=>setLocalSettings({...settings,address:e.target.value})}/></label><div className="form-actions full"><button className="primary"><Check/> حفظ الإعدادات</button></div></form></section>
    <section className="panel"><div className="panel-head"><div><h2>إدارة البيانات المحلية</h2><p>النسخة الحالية تحفظ البيانات داخل المتصفح، لذلك النسخ الاحتياطي مهم.</p></div></div><div className="row-actions"><button className="secondary" onClick={exportData}><DownloadSimple/> تنزيل نسخة احتياطية</button><button className="danger" onClick={resetData}>مسح بيانات التشغيل</button></div></section>
  </>;
}
...
async function localApi(path, options={}) {
  ...
  if (path === "/auth/login" && method === "POST") {
    ...
    if (!user || body.password !== (user.password || "01023299755")) throw new Error("بيانات الدخول غير صحيحة");
